    ...
    useEffect(() => {
        if (!usuarios) {
          usuarioService.listarTodos().then((response) => {
            ...
            setUsuarios(response.data);  // Atualiza o estado dos recursos
          }).catch((error) => {
            console.log(error);
          });
        }
        ...
        if(usuario.id){
            usuarioService.excluir(usuario.id).then((response) => {
                ...
                toast.current?.show({
                    severity: 'error',
                    summary: 'Erro!',
                    detail: 'Erro ao Alterar Usuario'
                });
            })
        }
    };
            ...
            Promise.all(selectedUsuarios.map(async (_usuario) => {
                if (_usuario.id !== undefined) {
                    try {
                        ...
                        console.error(`Erro ao excluir usuário com id ${_usuario.id}:`, error);
                        toast.current?.show({
                            severity: 'error',
                            summary: 'Erro!',
                            detail: `Erro ao excluir usuário com id ${_usuario.id}`
                        });
                    }
                }
            })).then(() => {
                setUsuarios((prevUsuarios) => prevUsuarios?.filter(usuario => !selectedUsuarios.includes(usuario)) || []);
                setSelectedUsuarios([]);
                setDeleteUsuariosDialog(false);
                toast.current?.show({
                    severity: 'success',
                ...
                <div className="card">
                    ...
                    <Dialog visible={usuarioDialog} style={{ width: '450px' }} header="Usuario Details" modal className="p-fluid" footer={usuarioDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.login && <small className="p-invalid">Email é Obrigatorio.</small>}
                        </div>
                        ...
                        <div className="field">
                            ...
                            />
                            {submitted && !usuario.telefone && <small className="p-invalid">Telefone é Obrigatorio.</small>}
                        </div>
